...
const getShoppingListsById = (id, done) => {
	model.shopping_List.findOne({ _id: id }, (err, data) => {
		if (err) return done(err);
			...
			if (data == null) return done(false);
			return done(null, data);
		}
	});
};
...
let minutes_prd = 10,
	the_interval_prd = minutes_prd * 60 * 1000;
 ...
 */
let minutes_remShoppingLists = 5,
	the_interval_remShoppingLists = minutes_remShoppingLists * 60 * 1000;
setInterval(() => {
	console.log(
...
//  *  In Case of an Error it will return an error, else it will return the ShoppingList as JSON
//  */
const addProductToShoppingList = (shoppingListId, groupname, productId, name, measure, amount, checked, done) => {
	model.shopping_List.findOne({ _id: mongoose.Types.ObjectId(shoppingListId) }, (err, data) => {
		...
		for (const key in data.productgroups) {
			...
			for (let i = 0; i < data.productgroups[key].products.length; i++) {
				if (data.productgroups[key].products[i]._id.equals(mongoose.Types.ObjectId(productId))) {
					return done(new Error('This Product is already in the shoppingList'))
				}
			}
			console.log(name)
			data.productgroups[key].products.push({ _id: mongoose.Types.ObjectId(productId), name: name, measure: measure, amount: amount, checked: checked })
		}
...
//  * Remove a Product by its ID from a shoppinglist (ID must be provided)
//  */
const removeProductFromShoppingList = (shoppingListId, groupname, productId, done) => {
	model.shopping_List.findOne({ _id: mongoose.Types.ObjectId(shoppingListId) }, (err, data) => {
		if (err) return done(err)
		else {
			if (data == undefined) return done(false)
...
const createShoppingListGroup = (shoppingListId, groupname, done) => {
	model.shopping_List.findOne({ _id: mongoose.Types.ObjectId(shoppingListId) }, (err, data) => {
		...
		data.productgroups.push({ group: groupname, group: groupname, products: [] })
	...
	model.shopping_List.findOne({ _id: mongoose.Types.ObjectId(shoppingListId) }, (err, data) => {
		...
		for (let key in data.productgroups) {
			...
			for (let iKey in data.productgroups[key].products) {
				...
				if (data.productgroups[key].products[iKey]._id.equals(mongoose.Types.ObjectId(updateObj._id))) {
...
//  * sets a intervall to remove all unused Shopping Lists every 10 minutes
//  */
const minutes_remShoppingLists = 5; const the_interval_remShoppingLists = minutes_remShoppingLists * 60 * 1000
setInterval(() => {
